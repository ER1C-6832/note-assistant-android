...
class AndroidAudioRecorder {
    ...
    data class AudioProcessingReport(
        ...
        private fun effectLabel(available: Boolean, enabled: Boolean): String = when {
            enabled -> "on"
    ...
    fun readPcmFrame(audioRecord: AudioRecord, frameBuffer: ByteArray): Boolean {
        var offset = 0
    ...
    private fun attachAudioProcessing(
        ...
        val aec = if (request.enableAec && aecAvailable) runCatching { AcousticEchoCanceler.create(sessionId) }.getOrNull() else null
        val noiseSuppressor = if (request.enableNoiseSuppressor && nsAvailable) runCatching { NoiseSuppressor.create(sessionId) }.getOrNull() else null
        val agc = if (request.enableAgc && agcAvailable) runCatching { AutomaticGainControl.create(sessionId) }.getOrNull() else null
        ...
        return RecordSession(
            ...
            processingReport = AudioProcessingReport(
                ...
                aecEnabled = enableEffect(aec),
                noiseSuppressorAvailable = nsAvailable,
                noiseSuppressorEnabled = enableEffect(noiseSuppressor),
                agcAvailable = agcAvailable,
                agcEnabled = enableEffect(agc),
            ),
            aec = aec,
